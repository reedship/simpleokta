  ...
  describe 'GROUP METHODS' do
    describe '#groups' do
      it 'returns an array' do
        VCR.use_cassette('groups/groups', match_requests_on: [:path]) do
    ...
    describe '#group' do
      it 'returns a hash when passed a valid id' do
        VCR.use_cassette('groups/group', match_requests_on: [:path]) do
      ...
      it 'returns an error when passed a valid id' do
        VCR.use_cassette('groups/invalid_group', match_requests_on: [:path]) do
    ...
    describe '#apps_assigned_to_group' do
      it 'returns an array when passed a valid id' do
        VCR.use_cassette('groups/group_apps', match_requests_on: [:path]) do
          response = client.apps_assigned_to_group('00gxf5kry6skrJf095d6')
          expect(response.class).to eq(Array)
        end
      end
      it 'returns an error when passed an invalid id' do
        VCR.use_cassette('groups/invalid_group_apps', match_requests_on: [:path]) do
          response = client.group('invalidid')
          expect(response['errorCode']).to eq('E0000007')
          expect(response['errorSummary']).to eq('Not found: Resource not found: invalidid (UserGroup)')
        end
    ...
    describe '#assign_group_to_application' do
      it 'returns an application group when passed valid ids' do
        VCR.use_cassette('groups/assign_group', match_requests_on: [:path]) do
      ...
      it 'returns an error when passed an invalid group id' do
        VCR.use_cassette('groups/invalid_assign_group_group', match_requests_on: [:path]) do
      ...
      it 'returns an error when passed an invalid app id' do
        VCR.use_cassette('groups/invalid_assign_group_app', match_requests_on: [:path]) do
    ...
    describe '#remove_group_from_application' do
      it 'returns a 204 status code when passed valid ids' do
        VCR.use_cassette('groups/remove_group_from_app', match_requests_on: [:path]) do
          response = client.remove_group_from_application('0oayt04y5o7ecjjU05d6', working_group_id)
          expect(response.code).to eq(204)
        end
      end
      it 'returns an error when passed an invalid group id' do
        VCR.use_cassette('groups/invalid_remove_group_from_app', match_requests_on: [:path]) do
      ...
      it 'returns an error when passed an invalid app id' do
        VCR.use_cassette('groups/invalid_remove_group_from_app_app', match_requests_on: [:path]) do
    ...
    describe '#get_assigned_group_for_application' do
      it 'returns a group assignment object when passed a valid app id' do
        VCR.use_cassette('groups/get_group_for_app', match_requests_on: [:path]) do
      ...
      it 'returns an error when passed an invalid app id' do
        VCR.use_cassette('groups/invalid_get_group_for_app', match_requests_on: [:path]) do
    ...
    describe '#update_group' do
      it 'returns the new group data when passed valid data' do
        VCR.use_cassette('groups/update_group', match_requests_on: [:path]) do
          group_data = {
            profile: {
      ...
      it 'returns an error when passed invalid data' do
        VCR.use_cassette('groups/invalid_update_group', match_requests_on: [:path]) do
          group_data = {
            boop: 'bap'
          }
          response = client.update_group(working_group_id, group_data)
    ...
    describe '#remove_group' do
      it 'returns a 204 when pased a valid group id' do
        VCR.use_cassette('groups/remove_group', match_requests_on: [:path]) do
          response = client.remove_group('00g11z4uzrfUJaw3d5d7')
          expect(response.code).to eq(204)
        end
      end
      it 'returns an error when passed an invalid group id' do
        VCR.use_cassette('groups/invalid_remove_group', match_requests_on: [:path]) do
    ...
    describe '#group_members' do
      it 'returns an array of User objects when passed a valid group id' do
        VCR.use_cassette('groups/group_members', match_requests_on: [:path]) do
          response = client.group_members(working_group_id)
          expect(response.class).to eq(Array)
        end
      end
      it 'returns an error when passed an invalid group id' do
        VCR.use_cassette('groups/invalid_group_members', match_requests_on: [:path]) do
    ...
    describe '#add_user_to_group' do
      it 'returns a 204 status code when pased a valid group id' do
        VCR.use_cassette('groups/add_user_to_group', match_requests_on: [:path]) do
          response = client.add_user_to_group(working_group_id, '00u10ribsoVWUttmX5d7')
          expect(response.code).to eq(204)
        end
      end
      it 'returns an error when passed an invalid group id' do
        VCR.use_cassette('groups/invalid_add_user_to_group', match_requests_on: [:path]) do
      ...
      it 'returns an error when passed an invalid user id' do
        VCR.use_cassette('groups/invalid_user_app_user_to_group', match_requests_on: [:path]) do
    ...
    describe '#remove_user_from_group' do
      it 'returns a 204 when pased a valid group id' do
        VCR.use_cassette('groups/remove_user_from_group', match_requests_on: [:path]) do
          response = client.remove_user_from_group(working_group_id, '00u10ribsoVWUttmX5d7')
          expect(response.code).to eq(204)
        end
      end
      it 'returns an error when passed an invalid group id' do
        VCR.use_cassette('groups/invalid_remove_user_from_group', match_requests_on: [:path]) do
      ...
      it 'returns an error when passed an invalid user id' do
        VCR.use_cassette('groups/invalid_user_remove_user_from_group', match_requests_on: [:path]) do
